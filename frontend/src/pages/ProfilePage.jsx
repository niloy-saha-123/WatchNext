 ...
 * @description User profile page with dynamic stats and favorite genres.
 */
...
function ProfilePage() {
  ...
  const { userInfo, stats, favoriteGenres, recentlyWatched } = profileData;
      ...
      <main className="pt-16">
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-20">
            <LoadingSpinner size="large" variant="primary" text="Loading your profile..." />
          </div>
        )}
                    ...
                    <div className="flex-shrink-0">
                      <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm border-2 border-white/30">
                        {userInfo.avatar ? (
                          ...
                          <span className="text-2xl font-bold text-white">{getInitials(userInfo.name)}</span>
                        )}
                      </div>
                    </div>
                    <div className="text-center md:text-left">
                      <h1 className="text-3xl md:text-4xl font-bold mb-2">
                        {isLoading ? '...' : userInfo.name || 'Guest User'}
                      </h1>
                      <p className="text-lg opacity-90 mb-1">
                        {isLoading ? '...' : userInfo.email || 'user@example.com'}
                      </p>
                      <p className="text-sm opacity-75">
                        {isLoading ? '...' : `Member since ${userInfo.memberSince || 'Recently'}`}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-12">
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
                    ...
                    {isLoading ? (
                      ...
                      <div className="text-center py-8 text-slate-500">
                        <p className="text-sm">No viewing data yet. Start watching to see your favorite genres!</p>
                      </div>
                    ) : (
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-8 shadow-sm text-center">
                  <div className="max-w-md mx-auto">
                    ...
                    <p className="text-slate-600 mb-4">
                      View your complete viewing history with detailed logs, ratings, and notes.
                    </p>
